...
`,f=()=>{const e=location.hostname.includes("github.io")?"/front_5th_chapter1-1":"",s=a.logIn(),o=location.hash?location.hash.replace(/^#/,""):window.location.pathname,n=s?`
    ...
    <header class="bg-blue-600 text-white p-4 sticky top-0">
      ...
      </nav>`},b=()=>{const t=a.getUser();return`
    <div id="root">
  ...
  </div > `},p=()=>{const t=document.getElementById("profile-form");t.addEventListener("submit",e=>{e.preventDefault();const s=new FormData(t);a.setUser({username:s.get("username")||"",email:s.get("email")||"",bio:s.get("bio")||""}),alert("프로필 업데이트가 완료되었어요.")})},h=()=>{document.body.innerHTML=b()},x=location.hostname.includes("github.io"),u=x?"/front_5th_chapter1-1":"",y=()=>`
...
`).join("");return`
  ...
  </main > `,L=location.hostname.includes("github.io"),l=L?"/front_5th_chapter1-1":"",d={[`${l}/`]:$,[`${l}/login`]:y,[`${l}/profile`]:b};function i(){const t=document.getElementById("root"),e=window.location.pathname,s=a.logIn(),r=d[e]||P;if(t){if(t.innerHTML=r(),e===`${l}/profile`&&!s){history.replaceState(null,"",`${l}/login`),t.innerHTML=d[`${l}/login`]();return}if(e===`${l}/login`&&s){history.replaceState(null,"",`${l}/`),t.innerHTML=d[`${l}/`]();return}if(e===`${l}/login`&&!s){v();return}e===`${l}/profile`&&s&&p()}}function S(t){history.pushState({},"",t),i()}const H=location.hostname.includes("github.io"),E=H?"/front_5th_chapter1-1":"";window.addEventListener("DOMContentLoaded",i);window.addEventListener("popstate",i);document.addEventListener("click",t=>{const e=t.target.closest("a[data-route-link]");if(t.target.closest("#logout")){t.preventDefault(),a.logout(),history.replaceState(null,"",`${E}/login`),i();return}e&&(t.preventDefault(),S(e.getAttribute("href")))});
